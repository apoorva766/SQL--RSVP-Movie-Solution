...
-- Total numbers of row is 3867
select count(*) from genre ;
-- total numbers for row is 14662
select count(*) from movie ;
-- Total numbers of row is 7997
select count(*) from names ;
-- Total numbers of row is 25735
select count(*) from ratings ;
-- Total numbers of row is 7997
select count(*) from role_mapping ; 
-- Total numbers of row is 15615
...
-- Query to count the number of null in each colunm using case statements
SELECT Sum(CASE
...
+---------------+-------------------+ */
-- Type your code below:
-- Number of movie released each year
select year,
...
select count(DISTINCT id) AS number_of_movie, year
from movie
where (country LIKE '%INDIA%'
        OR country LIKE '%USA%')
      AND year = 2019 ;  
-- In 2019 the total movies were produced in the USA or India is 1059
...
group by genre 
order by number_of_movie DESC limit 1 ;               

-- highest number of movies produced overall is 4285 in drama genre
...
-- Top 3 movies have average rating >= 9.6
...
-- By analysing, German movies get more votes than Italian movies 
